 ...
 */
async function getTagData(sqlite, topicId, id) {
	const tag = await getTagById(sqlite, topicId, id)
	if(!tag) return null
	return {
		id: tag.id,
		label: tag.label,
 ...
 */
async function canEditTag(sqlite, topicId, user, tagId) {
	if(!user) return false
	if(user.isAdmin) return true

	const coveredTagIds = await getDescendantIds(sqlite, topicId, tagId) // tagId + every more specific tag
	...
	const entries = await getEntriesByIds(sqlite, topicId, [...votedEntryIds])
 ...
 */
async function renameTag(sqlite, topicId, id, newLabel) {
	const tag = await getTagById(sqlite, topicId, id)
	if(!tag) return 'not_found'

	const trimmed = (newLabel || '').trim()
	if(!trimmed) return 'invalid'
	if(await getTagByLabelIgnoreCase(sqlite, topicId, trimmed, id)) return 'conflict'

	tag.label = trimmed
	await saveTag(sqlite, topicId, tag)
	return 'ok'
}
 ...
 */
async function getOrCreateTag(sqlite, topicId, label) {
	return getTagByLabel(sqlite, topicId, label, true)
}
 ...
 */
async function addTagParent(sqlite, topicId, tagId, newParentId) {
	return addParent(sqlite, topicId, tagId, newParentId)
}

/**
 * Removes the parent link. Returns 'not_found' | 'ok'.
 */
async function removeTagParent(sqlite, topicId, tagId, parentIdToRemove) {
	return removeParent(sqlite, topicId, tagId, parentIdToRemove)
}
 ...
 * tag.
 */
async function getParentTree(sqlite, topicId, tagId) {
	const tag = await getTagById(sqlite, topicId, tagId)
	if(!tag) return []

	const parentsById = await getTagsByIds(sqlite, topicId, tag.parents)
	const grandParentIds = [...parentsById.values()].flatMap((p) => p.parents)
	const grandParentsById = await getTagsByIds(sqlite, topicId, grandParentIds)
 ...
 */
async function getChildTree(sqlite, topicId, tagId) {
	const directChildren = await getDirectChildren(sqlite, topicId, tagId)
	const grandChildrenByParent = await getDirectChildrenForTagIds(sqlite, topicId, directChildren.map((c) => c.id))
 ...
 */
async function getEntriesForTag(sqlite, topicId, tagId, {user, sort, order} = {}) {
	const relevantTagIds = await getDescendantIds(sqlite, topicId, tagId) // includes tagId + every more specific tag
 ...
 */
async function isTagCoveredByEntry(sqlite, topicId, entry, tagId) {
	const descendants = await getDescendantIds(sqlite, topicId, tagId)
 ...
 */
async function addTagToEntry(sqlite, topicId, entryId, tagId) {
	const entry = await getEntryById(sqlite, topicId, entryId)
	const tag = await getTagById(sqlite, topicId, tagId)
	if(!entry || !tag) return 'not_found'
	if(await isTagCoveredByEntry(sqlite, topicId, entry, tagId)) return 'already_covered'
 ...
 */
async function removeTagFromEntry(sqlite, topicId, entryId, tagId) {
	const entry = await getEntryById(sqlite, topicId, entryId)
 ...
 */
async function resolveEntryTags(sqlite, topicId, entry) {
	const result = []
 ...
 */
async function searchTags(sqlite, topicId, {q, notOnEntity, notHavingAsParent, notHavingAsChild} = {}) {
	let candidates = q ? await searchTag(sqlite, topicId, q) : await getAllTags(sqlite, topicId)

	if(notOnEntity) {
		const entry = await getEntryById(sqlite, topicId, notOnEntity)
		if(entry) {
			const filtered = []
			for(const t of candidates) {
				if(!await isTagCoveredByEntry(sqlite, topicId, entry, t.id)) filtered.push(t)
			}
			candidates = filtered
		}
	}
	if(Array.isArray(notHavingAsChild)) {
		...
		for(const tagId of notHavingAsChild) {
			for(const descendantId of await getDescendantIds(sqlite, topicId, tagId)) excluded.add(descendantId)
		}
	...
	if(Array.isArray(notHavingAsParent)) {
		...
		for(const tagId of notHavingAsParent) {
			for(const ancestorId of await getAncestorIds(sqlite, topicId, tagId)) excluded.add(ancestorId)
		}
		candidates = candidates.filter((t) => !excluded.has(t.id))
	}
