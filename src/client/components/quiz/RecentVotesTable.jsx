...
function voteKey(vote) {
	return vote.type === 'default' ? 'default_' + vote.entry : 'dual_' + vote.neg + '_' + vote.pos
}
 ...
 * NewEntryForm/DualQuiz (last 5 of a single type, no pagination controls).
 *
 * `type` ('default' | 'dual', optional) filters to one quiz kind.
					...
					{items.map((vote) => {
						...
						return (
							<tr key={key} className={isBeingDeleted ? 'beingDeleted' : ''}>
								...
								<td>{vote.type}</td>
								<td>
									<div className="voteDetail">{vote.type === 'default'
